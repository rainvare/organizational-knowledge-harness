...
# Ensure root dir is in sys.path so parsers/, agents/, core/ are importable
# Works both locally and in HF Spaces (/app/)
...
.tag-color{background:rgba(220,180,80,.15);color:#dca830;border:1px solid #dca830;}
.tag-typography{background:rgba(80,180,200,.15);color:#50b4c8;border:1px solid #50b4c8;}
.tag-visual{background:rgba(180,120,200,.15);color:#b478c8;border:1px solid #b478c8;}
...
if "last_prompts" not in st.session_state:
    st.session_state.last_prompts = None
...
tabs = st.tabs(["01 · Ingesta","02 · Grafo","03 · Generación","04 · Coherencia","05 · Propuestas","06 · Exportar","07 · Historial","08 · Prompts"])
...
with t1:
    ...
    # source_text persists across button clicks via session_state
    if "source_text" not in st.session_state:
        st.session_state.source_text = ""
    if "source_modality" not in st.session_state:
        st.session_state.source_modality = "text"
    # Reset when user switches input method
    ...
    if input_method == "Texto / Pegar":
        ...
        url_input = st.text_input("URL", placeholder="https://www.behance.net/... · https://www.figma.com/... · cualquier URL")

        figma_token = ""
        with st.expander("🔑 Figma token (opcional — para archivos privados o extracción completa)"):
            figma_token = st.text_input("Personal Access Token de Figma", type="password",
                placeholder="fig_xxx...", help="figma.com → Settings → Account → Personal access tokens")

        if url_input:
            modality = "url"
            source_name = source_name or url_input

            from parsers.url_parser import detect_url_type
            url_type = detect_url_type(url_input)
            type_config = {
                "behance":  ("🎨", "Behance / Visual Portfolio", "var(--pu)", "Scraping + visión IA sobre imágenes del proyecto"),
                "figma":    ("✦",  "Figma",                      "var(--bl)", "oEmbed + thumbnail via visión IA. Con token: colores, tipografías, componentes"),
                "dribbble": ("🎨", "Dribbble",                   "var(--pu)", "Scraping + visión IA sobre imágenes"),
                "pinterest":("📌", "Pinterest",                   "var(--rd)", "Scraping + visión IA sobre imágenes del board"),
                "generic":  ("🔗", "URL genérica",               "var(--tm)", "Extrae texto, meta tags, headings"),
            }
            icon, label, color, hint = type_config.get(url_type, type_config["generic"])
            st.markdown(f'<div style="display:flex;align-items:center;gap:.6rem;margin:.4rem 0 .8rem;"><span style="background:rgba(150,150,150,.1);border:1px solid {color};color:{color};padding:.2rem .7rem;border-radius:2px;font-size:.75rem;">{icon} {label}</span><span style="font-size:.74rem;color:var(--tm);">{hint}</span></div>', unsafe_allow_html=True)

            if st.button("Analizar URL →", use_container_width=True):
                from parsers.router import route_url
                spinner_msg = {
                    "behance": "Analizando proyecto Behance (scraping + visión IA)...",
                    "figma":   "Procesando archivo Figma...",
                    "dribbble":"Analizando proyecto Dribbble...",
                }.get(url_type, "Extrayendo contenido...")
                with st.spinner(spinner_msg):
                    result = route_url(url_input, api_key=akey(), figma_token=figma_token)
                source_text = result.get("text", "")
                if result.get("error"): st.warning(result["error"])
                if result.get("images_analyzed"):
                    st.success(f"✓ {result['images_analyzed']} imagen(es) analizada(s) vía visión IA")
                if source_text:
                    st.session_state.source_text = source_text
                    st.session_state.source_modality = "url"
                    st.success(f"✓ {len(source_text):,} chars extraídos — {label}")
                    st.markdown(f'<div class="mc" style="font-size:.78rem;max-height:220px;overflow-y:auto;"><b style="color:var(--tm);">PREVIEW — {label.upper()}</b><br><pre style="white-space:pre-wrap;font-size:.74rem;">{source_text[:1000]}{"..." if len(source_text)>1000 else ""}</pre></div>', unsafe_allow_html=True)
                elif not result.get("error"):
                    st.error("No se pudo extraer contenido. El sitio puede requerir JavaScript o estar bloqueado.")

    col_types, col_btn = st.columns([2,1])
    with col_types:
        st.markdown('<div style="font-size:.75rem;color:var(--tm);margin-top:.5rem;">brand · audience · value · tone · restrict · example · color · typography · visual</div>', unsafe_allow_html=True)
    with col_btn:
        extract_btn = st.button("Extraer grafo →", use_container_width=True)

    if extract_btn:
        source_text = st.session_state.get("source_text", "")
        modality = st.session_state.get("source_modality", "text")
        if not source_text.strip():
            st.error("No hay contenido para extraer. Primero analizá la fuente.")
        elif not akey():
            ...
            if "error" in result:
                ...
                # Clear source_text after successful extraction
                st.session_state.source_text = ""
...
with t2:
    ...
    if not graph.get_nodes():
        ...
        with c1: ft = st.selectbox("Tipo",["todos"]+sorted(["brand","audience","value","tone","restrict","example","color","typography","visual"]))
        ...
        if st.session_state.get("show_add_node"):
            with st.form("add_node_form"):
                ...
                with nc1:
                    nn_type = st.selectbox("Tipo",["brand","audience","value","tone","restrict","example","color","typography","visual"])
...
# ============================================================
# TAB 08 — Prompts para imagen y video
# ============================================================
with t8:
    st.markdown("### Generador de prompts visuales")
    st.markdown('<p style="color:var(--tm);font-size:.82rem;">El grafo governa los prompts igual que governa el texto. Cada prompt respeta restricciones, aplica tono visual y mantiene coherencia con la identidad de marca.</p>', unsafe_allow_html=True)

    if not graph.get_nodes():
        st.markdown('<p style="color:var(--tm);">Grafo vacío. Ingesta fuentes primero para que el sistema conozca la marca.</p>', unsafe_allow_html=True)
    else:
        c1, c2 = st.columns([2, 1])
        with c1:
            prompt_brief = st.text_area(
                "Brief visual",
                height=100,
                placeholder="Ej: Una persona joven tomando una cerveza en un asado de domingo con amigos. Ambiente cálido, verano argentino, luz de tarde."
            )
        with c2:
            prompt_tool = st.selectbox(
                "Herramienta destino",
                ["Midjourney", "DALL-E 3", "Stable Diffusion", "Sora (video)", "Runway (video)", "Kling (video)"]
            )
            prompt_style = st.selectbox(
                "Estilo visual",
                ["Fotorrealista", "Cinematográfico", "Editorial", "Lifestyle", "Minimalista", "Ilustración"]
            )
            prompt_format = st.selectbox(
                "Formato",
                ["Horizontal 16:9", "Vertical 9:16 (stories)", "Cuadrado 1:1", "Cine 21:9"]
            )

        if st.button("Generar prompts →", use_container_width=True):
            if not prompt_brief.strip():
                st.error("Describí el brief visual.")
            elif not akey():
                st.error("Ingresa tu Groq API Key.")
            else:
                # Build brand context — ALL node types including formal identity
                brand_ctx = []
                brand = graph.get_nodes(node_type="brand")
                values = graph.get_nodes(node_type="value")
                tone = graph.get_nodes(node_type="tone")
                restrict = graph.get_nodes(node_type="restrict")
                examples = graph.get_nodes(node_type="example")
                colors = graph.get_nodes(node_type="color")
                typographies = graph.get_nodes(node_type="typography")
                visuals = graph.get_nodes(node_type="visual")

                if brand:
                    brand_ctx.append("BRAND: " + " | ".join(f"{n['label']}: {n['detail'][:80]}" for n in brand[:2]))
                if values:
                    brand_ctx.append("VALUES: " + " | ".join(n['label'] for n in values[:5]))
                if tone:
                    brand_ctx.append("TONE: " + " | ".join(f"{n['label']}: {n['detail'][:80]}" for n in tone[:2]))
                if colors:
                    brand_ctx.append("COLOR PALETTE:\n" + "\n".join(f"  - {n['label']}: {n['detail']}" for n in colors))
                if typographies:
                    brand_ctx.append("TYPOGRAPHY:\n" + "\n".join(f"  - {n['label']}: {n['detail']}" for n in typographies))
                if visuals:
                    brand_ctx.append("VISUAL SYSTEM:\n" + "\n".join(f"  - {n['label']}: {n['detail'][:100]}" for n in visuals[:4]))
                if restrict:
                    brand_ctx.append("NEVER:\n" + "\n".join(f"  - {n['label']}: {n['detail'][:80]}" for n in restrict))
                if examples:
                    brand_ctx.append("REFERENCE EXAMPLES: " + " | ".join(n['label'] for n in examples[:3]))

                brand_context = "\n".join(brand_ctx)

                system_prompt = f"""You are a visual prompt engineer and brand designer specializing in brand-consistent AI image and video generation.

Your task: transform a creative brief into optimized prompts for {prompt_tool}, strictly applying the brand's formal identity system.

BRAND IDENTITY SYSTEM:
{brand_context}

CRITICAL RULES:
- COLOR: Use ONLY the colors defined in COLOR PALETTE. Reference them by name and HEX in the prompt (e.g. "rich brown #754975 background").
- TYPOGRAPHY: If the brief involves text (flyers, posters, social), specify the exact typefaces from TYPOGRAPHY with their role (e.g. "Playfair Display serif headline, Montserrat sans-serif body text").
- VISUAL SYSTEM: Apply logo, layout, and photography rules from VISUAL SYSTEM.
- NEVER violate any restriction in the NEVER list.
- Style: {prompt_style}
- Format/Aspect ratio: {prompt_format}
- Optimize syntax specifically for {prompt_tool} (use native parameters, weights, syntax).

For pieces with text (flyers, banners, social posts): always specify font names, weights, and hierarchy explicitly in the prompt.
...
{{
  "prompts": [
    {{
      "version": "Principal",
      "prompt": "full optimized prompt — include specific colors with HEX, font names, layout rules",
      "negative_prompt": "what to exclude (colors outside palette, wrong fonts, etc.)",
      "parameters": "tool-specific parameters (e.g. --ar 16:9 --v 6 --style raw for MJ)",
      "notes": "which brand elements were applied and why"
    }},
    {{
      "version": "Alternativa A",
      "prompt": "...",
      "negative_prompt": "...",
      "parameters": "...",
      "notes": "..."
    ...
    {{
      "version": "Alternativa B — más arriesgada",
      "prompt": "...",
      "negative_prompt": "...",
      "parameters": "...",
      "notes": "..."
  ...
  "brand_alignment": "specific explanation of which colors, fonts, and visual rules were applied",
  "restrictions_applied": ["list each restriction that constrained the prompts"]
}}"""

                with st.spinner(f"Generando prompts para {prompt_tool}..."):
                    try:
                        ...
                        response = client.chat.completions.create(
                            ...
                            messages=[
                                ...
                                {"role": "user", "content": f"Brief: {prompt_brief}\nTool: {prompt_tool}\nStyle: {prompt_style}\nFormat: {prompt_format}"}
                            ],
                            temperature=0.8,
                            max_tokens=2000,
                        )
                        ...
                        st.session_state.last_prompts = result
                    except json.JSONDecodeError:
                        st.session_state.last_prompts = {"raw": response.choices[0].message.content}
                    except Exception as e:
                        st.error(str(e))

        # Display results
        if st.session_state.get("last_prompts"):
            result = st.session_state.last_prompts

            if "raw" in result:
                st.text_area("Respuesta raw", result["raw"], height=300)
            else:
                # Brand alignment
                if result.get("brand_alignment"):
                    st.markdown(f'''<div class="mc" style="border-left:3px solid var(--gn);margin-bottom:1rem;">
                        <span style="font-size:.7rem;color:var(--tm);">ALINEACIÓN CON MARCA</span><br>
                        <span style="font-size:.85rem;">{result["brand_alignment"]}</span>
                    </div>''', unsafe_allow_html=True)

                # Restrictions applied
                if result.get("restrictions_applied"):
                    restr = " · ".join(result["restrictions_applied"])
                    st.markdown(f'''<div style="font-size:.75rem;color:var(--tm);margin-bottom:1rem;">
                        <b style="color:var(--rd);">Restricciones aplicadas:</b> {restr}
                    </div>''', unsafe_allow_html=True)

                # Prompts
                for p in result.get("prompts", []):
                    version_color = {"Principal": "var(--ac)", "Alternativa A": "var(--bl)", "Alternativa B — más arriesgada": "var(--pu)"}.get(p.get("version",""), "var(--tm)")
                    st.markdown(f'''<div class="mc" style="border-left:3px solid {version_color};margin-bottom:1rem;">
                        <div style="font-size:.7rem;color:{version_color};margin-bottom:.5rem;font-weight:bold;">{p.get("version","").upper()}</div>
                    </div>''', unsafe_allow_html=True)
                    ...
                    st.text_area(
                        ...
                        value=p.get("prompt",""),
                        height=100,
                        key=f"prompt_{p.get('version','')}"
                    )
                    if p.get("negative_prompt"):
                        st.text_area(
                            "Negative prompt",
                            value=p.get("negative_prompt",""),
                            height=60,
                            key=f"neg_{p.get('version','')}"
                        )
                    if p.get("parameters"):
                        st.markdown(f'''<div style="font-family:'DM Mono',monospace;font-size:.8rem;color:var(--ac);background:var(--s);padding:.4rem .8rem;border-radius:2px;margin:.3rem 0;">{p.get("parameters","")}</div>''', unsafe_allow_html=True)
                    if p.get("notes"):
                        st.markdown(f'''<div style="font-size:.78rem;color:var(--tm);margin-bottom:1rem;padding-left:.5rem;border-left:1px solid var(--b);">{p.get("notes","")}</div>''', unsafe_allow_html=True)
                    st.markdown("---")
