    ...
    if not graph.get_nodes():
        ...
        if st.button("Generar prompts →", use_container_width=True):
            if not prompt_brief.strip():
                ...
                # Build brand context for visual prompts
                brand_ctx = []
                ...
                if brand:
                    brand_ctx.append("BRAND: " + " | ".join(f"{n['label']}: {n['detail'][:60]}" for n in brand[:2]))
                if values:
                    brand_ctx.append("VALUES: " + " | ".join(n['label'] for n in values[:4]))
                if tone:
                    brand_ctx.append("TONE: " + " | ".join(f"{n['label']}: {n['detail'][:50]}" for n in tone[:2]))
                if restrict:
                    brand_ctx.append("NEVER: " + " | ".join(n['label'] for n in restrict[:5]))
                if examples:
                    brand_ctx.append("EXAMPLES: " + " | ".join(n['label'] for n in examples[:3]))

                brand_context = "\n".join(brand_ctx)

                system_prompt = f"""You are a visual prompt engineer specializing in brand-consistent AI image and video generation.

Your task: transform a creative brief into optimized prompts for {prompt_tool}, strictly respecting brand guidelines.

BRAND GUIDELINES:
{brand_context}

RULES:
- Every prompt must visually reflect the brand values
- Never include anything from the NEVER list
- Style: {prompt_style}
- Format: {prompt_format}
- Optimize syntax specifically for {prompt_tool}
...
{{
  "prompts": [
    {{
      "version": "Principal",
      "prompt": "full optimized prompt here",
      "negative_prompt": "what to exclude (for SD/Midjourney)",
      "parameters": "tool-specific parameters (e.g. --ar 16:9 --v 6 for MJ)",
      "notes": "why this prompt works for the brand"
    }},
    {{
  ...
  "brand_alignment": "brief explanation of how these prompts align with brand values",
